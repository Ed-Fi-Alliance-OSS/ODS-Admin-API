...
public class AddApplicationCommand(IUsersContext usersContext) : AddApplicationCommandBase(usersContext), IAddApplicationCommand
{
    public AddApplicationResult Execute(IAddApplicationModel applicationModel, IOptions<AppSettings> options)
    {
        return ExecuteCore(applicationModel, options);
    }
}
