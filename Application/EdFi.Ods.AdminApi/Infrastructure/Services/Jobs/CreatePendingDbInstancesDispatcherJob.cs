    ...
    protected override async Task ExecuteJobAsync(IJobExecutionContext context)
    {
        ...
        try
        {
            ...
            var eligibleDbInstances = await adminApiDbContext.DbInstances
                .Where(instance => instance.Status == DbInstanceStatus.Pending.ToString() || instance.Status == DbInstanceStatus.Error.ToString())
                .OrderBy(instance => instance.Id)
                .ToListAsync();

            foreach (var dbInstance in eligibleDbInstances)
            {
                if (string.Equals(dbInstance.Status, DbInstanceStatus.Pending.ToString(), StringComparison.OrdinalIgnoreCase))
                {
                ...
                dbInstance.Status = DbInstanceStatus.Pending.ToString();
