...
namespace EdFi.Ods.AdminApi.Common.Infrastructure.Services.EducationOrganizationService;

public abstract class EducationOrganizationServiceBase(
    IOptions<AppSettings> options,
    IUsersContext usersContext,
    ISymmetricStringEncryptionProvider encryptionProvider,
    IServiceScopeFactory serviceScopeFactory,
    ILogger logger,
    Func<string, IUsersContext> getTenantUsersContext,
    Func<IServiceProvider, string, AdminApiDbContext> getTenantAdminApiDbContext) : IEducationOrganizationServiceCore
{
   ...
   ";
    ...
    public async Task Execute(string? tenantName, int? instanceId)
    {
        ...
        if (multiTenancyEnabled)
        {
            ...
            var tenantSpecificUsersContext = getTenantUsersContext(tenantName);
    ...
    public virtual async Task ProcessOdsInstanceAsync(string? tenantName, IUsersContext usersContext, string encryptionKey, string databaseEngine, int? instanceId = null, int maxDegreeOfParallelism = 10)
    {
        ...
        await Parallel.ForEachAsync(
            ...
            async (odsInstance, cancellationToken) => { await RefreshEducationOrganizationsAsync(tenantName, encryptionKey, databaseEngine, odsInstance, cancellationToken); });
    }

    protected virtual async Task RefreshEducationOrganizationsAsync(string? tenantName, string encryptionKey, string databaseEngine, OdsInstance odsInstance, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // Isolate DbContext per task to avoid cross-thread usage.
            ...
            await using var taskAdminApiDbContext = tenantName is null
                ...
                : getTenantAdminApiDbContext(scope.ServiceProvider, tenantName);
    ...
    public virtual async Task<List<EducationOrganizationResult>> GetEducationOrganizationsAsync(string? connectionString, string databaseEngine, CancellationToken cancellationToken = default)
    {
        ...
        if (databaseEngine.Equals(DatabaseEngineEnum.PostgreSql, StringComparison.OrdinalIgnoreCase))
        {
            using var connection = new NpgsqlConnection(connectionString);
            await connection.OpenAsync(cancellationToken);
            using var command = new NpgsqlCommand(AllEdorgQuery, connection);
            using var reader = await command.ExecuteReaderAsync(cancellationToken);
            return await ReadEducationOrganizationsFromDbDataReader(reader, cancellationToken);
        }
    ...
    private async Task<List<EducationOrganizationResult>> ReadEducationOrganizationsFromDbDataReader(DbDataReader reader, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            while (await reader.ReadAsync(cancellationToken))
            {
                try
                {
                    var educationOrganizationId = ConvertToLong("educationorganizationid");
                ...
                {
                    logger.LogError(ex, "Data conversion error while reading education organizations. {Error}", ex.Message);
                }
            }
        }
        catch (Exception ex)
        ...
        long ConvertToLong(string columnName)
        {
...
public interface IEducationOrganizationServiceCore
{
