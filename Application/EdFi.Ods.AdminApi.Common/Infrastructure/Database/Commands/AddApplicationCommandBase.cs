...
public abstract class AddApplicationCommandBase(IUsersContext usersContext)
{
    private readonly IUsersContext _usersContext = usersContext;

    protected AddApplicationResult ExecuteCore(IAddApplicationModel applicationModel, IOptions<AppSettings> options)
    {
        if (options.Value.PreventDuplicateApplications)
        {
            var validateApplicationExists = new ValidateApplicationExistsQuery(_usersContext);
