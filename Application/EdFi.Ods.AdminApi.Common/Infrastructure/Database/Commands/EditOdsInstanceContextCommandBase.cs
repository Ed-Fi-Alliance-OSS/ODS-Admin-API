...
public abstract class EditOdsInstanceContextCommandBase(IUsersContext context)
{
    private readonly IUsersContext _context = context;

    protected OdsInstanceContext ExecuteCore(int id, int odsInstanceId, string? contextKey, string? contextValue)
    {
        var odsInstanceContext = _context.OdsInstanceContexts
            .Include(oid => oid.OdsInstance)
            .SingleOrDefault(v => v.OdsInstanceContextId == id) ??
        ...
        odsInstanceContext.ContextKey = contextKey;
        odsInstanceContext.OdsInstance = odsInstance;
        odsInstanceContext.ContextValue = contextValue;
