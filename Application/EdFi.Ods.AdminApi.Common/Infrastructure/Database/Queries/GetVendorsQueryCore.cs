...
public class GetVendorsQueryCore
{
    private readonly IUsersContext _context;
    private readonly IOptions<AppSettings> _options;
    private readonly Dictionary<string, Expression<Func<Vendor, object>>> _orderByColumnVendors;

    public GetVendorsQueryCore(IUsersContext context, IOptions<AppSettings> options)
    {
        _context = context;
        _options = options;
...
#pragma warning restore CS8602
    ...
    public List<Vendor> Execute()
    {
        return _context.Vendors
            ...
            .OrderBy(v => v.VendorName).Where(v => !VendorExtensionsCore.ReservedNames.Contains(v.VendorName.Trim())).ToList();
    }
    ...
    public List<Vendor> Execute(CommonQueryParams commonQueryParams, int? id, string? company, string? namespacePrefixes, string? contactName, string? contactEmailAddress)
    {
        ...
        return _context.Vendors
            ...
            .Include(v => v.Users)
            .Include(v => v.VendorNamespacePrefixes)
            .Where(v => !VendorExtensionsCore.ReservedNames.Contains(v.VendorName.Trim()))
