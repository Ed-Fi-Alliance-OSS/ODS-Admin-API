...
public interface IEditResourceOnClaimSetModelCommon
{
    int ClaimSetId { get; }
    ResourceClaim? ResourceClaim { get; }
}

public abstract class EditResourceOnClaimSetCommandBase(ISecurityContext context)
{
    private readonly ISecurityContext _context = context;

    protected void ExecuteCore(IEditResourceOnClaimSetModelCommon model)
    {
        var resourceClaimToEdit = model.ResourceClaim;
        if (resourceClaimToEdit is null)
        {
            return;
        }
    ...
    private void AddEnabledActionsToClaimSet(ResourceClaim modelResourceClaim,
        IReadOnlyCollection<ClaimSetResourceClaimAction> claimSetResourceClaimsToEdit,
        EdFi.Security.DataAccess.Models.ClaimSet claimSetToEdit)
    {
