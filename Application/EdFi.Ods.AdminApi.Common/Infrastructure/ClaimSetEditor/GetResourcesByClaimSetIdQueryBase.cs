...
public abstract class GetResourcesByClaimSetIdQueryBase(ISecurityContext securityContext)
{
    private readonly ISecurityContext _securityContext = securityContext;

    protected IList<ResourceClaim> AllResourcesCore(int securityContextClaimSetId)
    {
    ...
    protected ResourceClaim? SingleResourceCore(int claimSetId, int resourceClaimId)
    {
        var parentResources = AllResourcesCore(claimSetId).ToList();
    ...
    internal IList<ResourceClaim> GetParentResources(int claimSetId)
    {
        var dbParentResources = _securityContext.ClaimSetResourceClaimActions
            ...
            .Where(x => x.ClaimSet.ClaimSetId == claimSetId
                        && x.ResourceClaim.ParentResourceClaimId == null)
            .ToList();
        ...
        parentResources.ForEach(x => x.Children = []);
        return parentResources;
    }

    public Dictionary<int, List<ClaimSetResourceClaimActionAuthStrategies?>> GetDefaultAuthStrategies(
        IReadOnlyCollection<SecurityResourceClaim> resourceClaims)
    {
        ...
        var defaultAuthStrategiesForParents = defaultAuthStrategies
            .Where(x => x.ResourceClaim.ParentResourceClaimId == null)
            .ToList();
        ...
        foreach (var resourceClaim in resourceClaims)
        {
            ...
            foreach (var action in _securityContext.Actions)
            {
                ...
                {
                    List<AuthorizationStrategy>? childResourceStrategies;
    ...
    private static List<AuthorizationStrategy> AddStrategiesToChildResource(
        IEnumerable<SecurityAuthorizationStrategy>? authStrategies,
    ...
    private Dictionary<int, List<ClaimSetResourceClaimActionAuthStrategies?>> GetAuthStrategyOverrides(
        List<ClaimSetResourceClaimAction> resourceClaims)
    {
        var resultDictionary = new Dictionary<int, List<ClaimSetResourceClaimActionAuthStrategies?>>();
        ...
        foreach (var resourceClaim in resourceClaims)
        {
            ...
            else
            {
                ...
                var parentResourceOverride = parentResources.SingleOrDefault(x =>
                    x.ResourceClaim.ResourceClaimId == resourceClaim.ResourceClaim.ParentResourceClaimId
                ...
                if (parentResourceOverride?.AuthorizationStrategyOverrides != null
                    && parentResourceOverride.AuthorizationStrategyOverrides.Count != 0)
                {
    ...
    private static List<AuthorizationStrategy> AddStrategyOverridesToResource(
        IEnumerable<ClaimSetResourceClaimActionAuthorizationStrategyOverrides>? authStrategies,
        bool fromParent = false)
    {
        var strategies = new List<AuthorizationStrategy>();
        if (authStrategies != null && authStrategies.Any())
