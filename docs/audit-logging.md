...
Every `Delete*Command`, in **both V2 (`EdFi.Ods.AdminApi`) and V3 (`EdFi.Ods.AdminApi.V3`)** — independently-implemented command classes per version, each taking the dependency individually; the DI registration itself is shared, not duplicated — takes a scoped `IDeletedEntityAuditCapture` and calls `Record(entity)` immediately after loading the entity it's about to remove. `AuditActionLoggingMiddleware` reads `IDeletedEntityAuditCapture.CapturedJson` after `next()` completes and passes it into `IAuditEventRecorder.Record(...)`.
...
**Refused deletes and snapshots.** A `DeletedObjectSnapshot` is recorded even when the delete itself is later refused (e.g. a system-reserved Vendor/Application/ClaimSet is rejected before removal completes, or an OdsInstanceManage delete is blocked because its status isn't `Created`). This is intentional: the capture call fires immediately after the entity loads, before any validation or permission checks that might reject the delete, so the snapshot records what was *targeted* — consistent with how `Action` events already record all outcomes (2xx, 4xx, 5xx), not just successes. The `StatusCode` column (already documented above) tells you whether the delete actually succeeded — a reader with a `DeletedObjectSnapshot` and a 400 or 403 `StatusCode` should interpret that as "this delete was attempted and blocked," not "this object was deleted."

**V1 mode.** Not covered — `AuditActionLoggingMiddleware` isn't registered when `AdminApiMode.V1` is active (the guard `if (adminApiMode is AdminApiMode.V2 or AdminApiMode.V3)` is in `Application/EdFi.Ods.AdminApi/Program.cs`, around lines 68–72), and V1's delete commands operate on a separate, repo-local model rather than the shared package types the registry covers.
