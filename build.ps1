...
<#
    ...
    .EXAMPLE
        ...
        Useful for testing locally with real version values in the GET /
        endpoint's "version" ("2.4.0") and "build" ("2.4.0.0") fields, instead of
        the default "0.1.0"/"0.1.0.0". Remember to revert Directory.Build.props
...
function SetAdminApiAssemblyInfo {
    Invoke-Execute {
        ...
        # V1/V3/Common ship together with the base project as a single product release, so every
        # project's assembly version is stamped identically here rather than only the base project's.
        $projectsToStamp = @(
            "EdFi.Ods.AdminApi",
        ...
        foreach ($project in $projectsToStamp) {
            Invoke-RegenerateFile "$solutionRoot/$project/Directory.Build.props" @"
<Project>
